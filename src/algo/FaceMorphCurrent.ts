// FROZEN BASELINE -- a verbatim copy of renderer/lib/faceMorph.ts from the
// production app (v3.0.0), so every comparison in this PoC is against the real
// thing rather than a paraphrase of it.
//
// Four deliberate deviations, all mechanical:
//   - renamed to FaceMorphCurrent so both implementations can be loaded at once
//   - imports types from ./types instead of ./protocol
//   - `render` takes MorphSource instead of HTMLVideoElement, so the batch
//     harness can feed decoded frames (HTMLVideoElement is still assignable)
//   - asset paths point at this project's vendored copies
// The morph maths below is untouched. Do not "fix" anything in this file --
// its whole job is to be the thing being measured.
...
import type { ExpressionLabel, ExpressionState, MorphSource, SmileType } from './types'
