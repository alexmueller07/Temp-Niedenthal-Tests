...
The one that dominates is **head turn**. The morph's yaw gate fades the
...
turning to speak. Head roll leaks into that gate too, because the gate is


estimated from screen-x distances, which roll changes.





**What that simulation licenses, and what it does not.** The table comes from


rotating each face's 3D landmark cloud and re-projecting it through a plausible


webcam focal length. That is fair for the *geometry* — where the landmarks end


up — which is what the yaw gate reads, so the result stands. It says nothing


about *appearance*: a turned head also hides part of itself and shades


differently, and no landmark rotation reproduces that.





Re-projecting the rendered image instead does not substitute. A camera-rotation
...
as about 7° of yaw plus 10° of spurious roll — so those conditions are excluded


from §10. Roll and distance *are* exact by the same check: 15° in, 15.6° out, and


scale exact to the commanded factor.





So the rendered behaviour under a real head turn is still untested. The fix is


cheap: the available dev clips are 17–20 frames with a yaw range of 0–4°, so
...
**Measured across the preset range** (12 faces, normalized implementation):



| alpha | smile units | commanded strain | median stretch | frames over 2x |

|---|---|---|---|---|

| 1.20 | 0.13 | 0.24 | 1.21x | 0/12 |

| 1.35 | 0.23 | 0.42 | 1.46x | 0/12 |

| **1.50** | **0.32** | **0.60** | **1.75x** | **1/12** |

| 1.65 | 0.42 | 0.78 | 2.14x | 7/12 |

| 1.90 | 0.58 | 1.08 | 2.72x | 9/12 |



**Recommendation: the strong preset should be about alpha 1.5, not 1.9.** That is

where the commanded deformation matches a real closed-mouth smile (strain 0.60

against 0.65) and where rendered stretch stays under 2x on all but one face in

twelve. At 1.9 the manipulation is larger than most people's own closed-mouth

smile, which is both why the RAs called it uncanny and why it visibly stretches
...
## 12. What an independent detector sees



Everything above is measured with MediaPipe, which is also what steers the

normalized morph — so it cannot settle which implementation is better. OpenFace

2.2 can: a different face model (CE-CLM) and a separately trained action-unit

regressor. AU12 is the lip corner puller, the action a smile is made of.



First, does the instrument work on these faces at all? Real neutral to real

closed-mouth smile, same 30 people, same framing: **dAU12 = +2.24, positive in

30/30**. Yes.



Now the morph, at alpha 1.9, each render differenced against a sham render of

the identical frame:



| | mean dAU12 | positive | dispersion of the self-referenced dose (CV) |

|---|---|---|---|

| current | +0.466 | 29/30 | 0.570 |

| normalized | +0.494 | 29/30 | **0.457** |
...
**The direction holds under an independent instrument, at a smaller effect

size.** OpenFace agrees the normalized morph is more equal across faces, but

reports a 20% reduction where MediaPipe reported 52%. Some of that gap is

OpenFace's own noise — its AU12 output is quantized in steps of about 0.05,

which inflates dispersion — but the honest summary is that the improvement is

real and smaller than the in-house number suggests. Quote the 20%.



**The manipulation is much weaker than its geometry implies.** At the strongest

preset it moves the mouth corners by 58% of a full smile's travel, but produces

only **21% of the AU12 change a real smile of that size produces**. A 2D warp

moves landmarks; it does not deepen a nasolabial fold, raise a cheek, or change

how light falls on any of it, and an AU detector reads all of those. If a human

rater weighs appearance the way an AU regressor does, the perceived manipulation

is a good deal smaller than the displacement suggests — which would explain why

a preset had to be pushed to a strain a real smile barely reaches before the RAs

felt it was doing anything.



That is an argument for the appearance layer this PoC deliberately did not build

(§Limitations), and an argument against reading the geometric numbers as if they

were perceptual ones.
