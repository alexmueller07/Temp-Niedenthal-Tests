...
def fig_views(rows: list[dict], alpha: float, out: Path) -> None:
    ...
    # Out-of-plane views are dropped, not plotted greyed: re-projecting a flat
    # photograph keystones it rather than turning a head, so those points say
    # nothing about pose invariance and would dominate the axis if shown.
    views = sorted({r["view"] for r in rows
                    if "yaw" not in r["view"] and "pitch" not in r["view"]
                    and not r["view"].startswith("keystone")})
    if len(views) < 2:
        return
    preferred = ["base", "near", "far", "low-in-frame",
                 "roll-15", "roll+15", "roll+25"]
    order = [v for v in preferred if v in views] +             [v for v in views if v not in preferred]
    ...
    ax.set_title("Same face, different camera: the dose should not move" + "\n" +
                 "distance, framing and roll only — exact image operations, "
                 "verified by re-detecting the pose",
                 loc="left", fontsize=9.5)
    fig.tight_layout()
    fig.savefig(out, bbox_inches="tight")
    plt.close(fig)
