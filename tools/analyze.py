...
def main() -> int:
    ...
    for run_id in args.run:
        ...
        # Only the views that are exact image operations count toward the
        # invariance claim; the keystone ones distort the face in a way a real
        # head turn does not, so they are reported but excluded from the statistic.
        # Matched on semantics rather than the label, so runs made before the
        # views were renamed are classified the same way.
        exact_views = {v for v in views
                       if "yaw" not in v and "pitch" not in v
                       and not v.startswith("keystone")}
        if len(views) > 1:
            print()
            print(f"--- within-face dispersion across {len(exact_views)} exact"
                  f" camera views at alpha {args.alpha} ---")
            print("    roll, distance and framing only. The out-of-plane views")
            print("    are listed below but excluded: re-projecting a flat")
            print("    photograph keystones it, it does not turn a head.")
            for impl in ("current", "normalized"):
